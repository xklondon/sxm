...
- **Card View settlement results:** mobile and desktop render every settled hand in canonical box order (`orderedHandKeys`); results are never truncated by a display-count cap.
...
| Card placement | `blackjackCardPlacementContract.ts` | Per-mode placement inside the cards zone only: Full Table = box-column stacks anchored just above box value; Card View = centered hero. Shell owns zone geometry; inner card CSS must not use one generic rule for all modes or `overflow:hidden` as a clipping workaround. **Frozen** — do not modify unless user explicitly requests. |
| Game over | `blackjackGameOverContract.ts`, `GameOverActionOverlay` | One centered modal for all views; side-rail game-over duplicate forbidden. |
