...
describe('Desktop Card View hero card clipping guards', () => {
  ...
  it('centers Card View desktop hero cards in grid row 3; mobile stays top-aligned', () => {
    ...
    expect(HERO_AREA_CSS).toMatch(
      /\.bj-view-card-desktop \.bj-card-desktop-hero__card\.ds-card--hero[\s\S]*max-height:\s*var\(--bj-card-hero-card-max-height\)/,
    );
    expect(HERO_AREA_CSS).toMatch(
    ...
    expect(HERO_AREA_CSS).toMatch(
      /\.bj-view-card-desktop \.bj-card-desktop-hero[\s\S]*height:\s*100%/,
    );
  });
});
