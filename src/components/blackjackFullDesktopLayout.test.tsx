...
describe('desktop Full Table layout polish', () => {
  ...
  it('spreads Full Table player boxes with fixed width columns across felt width', () => {
    expect(PLAYER_ROW_CSS).toMatch(
      /\.bj-view-full-desktop \.bj-table-slot-row\.bj-arc--player-boxes[\s\S]*justify-content:\s*(?:space-evenly|var\(--bj-desktop-player-row-spread,\s*space-evenly\))/,
    );
    expect(PLAYER_ROW_CSS).toMatch(
  ...
  it('fills flexible card area and bottom-pins columns on Full Table desktop', () => {
    ...
    expect(SHELL_CSS).toMatch(
      /@media \(min-width: 721px\)[\s\S]*\.bj-view-full-desktop \.bj-table-layout-shell > \.bj-table-zone--cards\.bj-cards-area--table[\s\S]*overflow-y:\s*visible/,
    );
  });
