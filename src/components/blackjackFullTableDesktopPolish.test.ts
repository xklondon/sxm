...
describe('desktop Full Table layout polish', () => {
  it('tightens Hit/Stay to ~5px above player box amount labels on desktop Full Table only', () => {
    expect(CARD_AREA_CSS).toMatch(
      /@media \(min-width: 721px\)[\s\S]*\.bj-view-full-desktop[\s\S]*--bj-full-desktop-actions-boxes-gap:\s*0\.3125rem/,
    );
    expect(CARD_AREA_CSS).toMatch(
