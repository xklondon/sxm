...
describe('Full Table card area canonical contract', () => {
  ...
  it('fills command-to-actions gap and bottom-aligns columns (desktop + mobile)', () => {
    ...
    expect(SHELL_CSS).toMatch(
      /\.bj-view-full-desktop \.bj-table-layout-shell > \.bj-table-zone--cards[\s\S]*flex:\s*unset/,
    );
    expect(SHELL_CSS).toMatch(
  ...
  it('does not apply Full Table card-column grid to Card View', () => {
    expect(CARD_AREA_CSS).not.toMatch(/\.bj-view-card-desktop[^{]*\.bj-arc__slot--card-column/);
    expect(CARD_AREA_CSS).not.toMatch(/\.bj-view-card-mobile[^{]*\.bj-arc__slot--card-column/);
    expect(CARD_AREA_CSS).not.toMatch(/\.bj-table-zone--cards\.bj-cards-area--hero[\s\S]*bj-arc__slot--card-column/);
  ...
  it('does not override desktop grid cards row — flexible 1fr stays in shell owner', () => {
