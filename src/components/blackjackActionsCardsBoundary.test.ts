...
describe('blackjack canonical shell order — Dealer → Actions → Command → Cards', () => {
  ...
  it('places Hit/Stay directly after dealer zone in desktop grid', () => {
    ...
    expect(desktop).toMatch(/\[dealer\][\s\S]*\[actions\][\s\S]*\[command\][\s\S]*\[cards\]/);
  ...
  it('keeps Actions→Command gap smaller than Command→Cards gap', () => {
    ...
    expect(SHARED_CSS).toContain('--bj-command-cards-safe-gap: 0.75rem');
    expect(SHARED_CSS).toMatch(
      /@media \(max-width: 720px\)[\s\S]*--bj-command-cards-safe-gap:\s*0\.55rem/,
    );
    expect(cards).toMatch(/margin-top:\s*var\(--bj-command-cards-safe-gap\)/);
    expect(SHARED_CSS).toMatch(
      /\.bj-table-layout-shell \.bj-table-zone--summary[\s\S]*padding-top:\s*var\(--bj-actions-command-gap\)/,
    );
    expect(SHARED_CSS).toMatch(
      /\.bj-table-layout-shell \.bj-table-zone--actions[\s\S]*padding-top:\s*var\(--bj-dealer-actions-gap\)/,
    );
  });

  it('anchors compact action panel without overlapping command or cards', () => {
    expect(SHARED_CSS).toMatch(
    ...
    expect(SHARED_CSS).toMatch(/\.bj-table-layout-shell > \.bj-table-zone--actions[\s\S]*z-index:\s*3/);
    expect(SHARED_CSS).toMatch(/\.bj-table-layout-shell > \.bj-table-zone--summary[\s\S]*z-index:\s*4/);
    expect(SHARED_CSS).toMatch(/\.bj-table-layout-shell > \.bj-table-zone--cards[\s\S]*z-index:\s*5/);
  });

  it('reveals hero card tops below command in Card View', () => {
