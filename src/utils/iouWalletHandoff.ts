...
export type IouWalletType = 'cash' | 'personal';
...
const CASH_WAGER_PATTERN =
  /(?:\$|€|£|¥|usd|eur|gbp|cash|money|dollar|euro|pound|bucks|\d+\s*(?:usd|eur|gbp))/i;

export function detectIouTypeFromWager(wager: string): IouWalletType {
...
export interface IouWalletPrefillParams {
...
export function buildIouWalletNewUrl(params: IouWalletPrefillParams): string {
...
export interface GameEndIouHandoff {
  counterpartyEmail: string;
  url: string;
}
