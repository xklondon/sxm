...
import { getCallerPersonIdForBox, resolveViewerPersonId, } from './playerAssignment';
...
function twoPlayerTable() {
    ...
    state = {
        ...
        tableMeta: {
            ...state.tableMeta,
            boxSlots: state.tableMeta.boxSlots.map((s) => s.slotNumber === 2
...
describe('resolveViewerPersonId', () => {
    ...
    it('active box 2: guest sees actionable hand, owner does not', () => {
        ...
        const round = actingRound(state, box2, [findCardId(state.deck, '10'), findCardId(state.deck, '9')], 10);
