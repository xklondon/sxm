...
import { allocateChipsToBankrollOwner, logDerivedBalances, logLedgerAfterAllocation, } from './allocation';
import { listPersonBankrollOwnerIds } from './bankroll';
export const ALL_BOXES_RECIPIENT = '__all_boxes__';
export function isTableOwner(state, personName) {
...
export function getStartingChipsEachSeat(state) {
...
export function getStartingChipsBank(state) {
...
export function logTableMetaStartingChips(state, context) {
...
function mapAssignReason(reason) {
    switch (reason) {
        case 'adjustment':
            return 'adjustment';
...
export function assignChips(state, recipientId, amount, reason, note) {
...
export function assignChipsToPlayer(state, playerId, amount, reason = 'top-up', note) {
    return assignChips(state, playerId, amount, reason, note);
}
/** @deprecated Use assignChips */
export function assignTokensToPlayer(state, playerId, amount, note) {
    return assignChips(state, playerId, amount, 'top-up', note);
}
export function logSetupValues(stakeDescription, startingChipsEachSeat, startingChipsBank) {
...
export { allocateChipsToParticipant, allocateChipsToBankrollOwner, createParticipantWithAllocation, logLedgerAfterAllocation, logDerivedBalances, logAccountPanelBalances, } from './allocation';
