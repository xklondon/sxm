...
import { createEmptyBoxSlots, MAX_TABLE_BOXES } from '../../types/table';
import { log } from '../../utils/logger';
import { derivePlayerBalanceFromLedger } from '../ledger/ledger';
import { addPlayer, assignBankOrDealer, mergeSessionUpdate, removePlayer, } from './session';
import { allocateChipsToBankrollOwner, createParticipantWithAllocation, logDerivedBalances, logLedgerAfterAllocation, } from './allocation';
...
export function setControllerName(state, name) {
...
function syncBankParticipantId(state, bankId) {
    return {
        ...state,
...
export function assignBankBot(state, startBalance) {
    ...
    const created = createParticipantWithAllocation(state, {
        displayName: 'Bank Bot',
        controllerName: 'Bank Bot',
        role: 'bank',
    }, chips, 'starting-allocation', 'setup');
...
export function assignBankPerson(state, personName, startBalance) {
...
export function claimBoxSlot(state, slotNumber) {
    ...
    const boxSlots = next.tableMeta.boxSlots.map((s) => s.slotNumber === slotNumber
        ? {
            ...
            bankrollOwnerId,
            nativeAssignedPersonId: s.nativeAssignedPersonId ??
...
export function releaseBoxSlot(state, slotNumber) {
    const slot = slotByNumber(state, slotNumber);
    if (!slot?.playerId) {
        return state;
    }
    ...
    const boxSlots = state.tableMeta.boxSlots.map((s) => {
        ...
        const nativeId = Object.entries(state.tableMeta.assignedBoxByPersonId ?? {}).find(([, slot]) => slot === slotNumber)?.[0] ?? s.nativeAssignedPersonId;
        return {
            ...s,
            playerId: null,
    ...
    const nextSelected = state.selectedSeatId === playerId ? null : state.selectedSeatId;
...
export function registerPassiveBet(state, boxPlayerId, bettorName) {
...
export function startNewGameWithWager(state, stakeDescription, startingChipsEachSeat, startingChipsBank) {
...
export function getSlotForPlayer(state, playerId) {
...
export function boxLabelForPlayer(state, playerId) {
    const num = state.session.boxSlotNumbers?.[playerId];
