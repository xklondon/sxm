...
function resolveOwnerName(state) {
    return (state.tableMeta.owner?.ownerName?.trim() ||
        state.tableMeta.controllerName.trim());
}
export function logThisTableRowsAfterSetup(state, context) {
 ...
 */
export function ensureTableOwnerPersonBankroll(state) {
