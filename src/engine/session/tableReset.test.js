...
import { startBlackjackRound, shuffleToStartOnState, dealCardsButtonOnState, completeStepwiseInitialDealIfNeeded, } from '../blackjack/gameState';
...
import { parseTableStakeSetupPayload } from './tableSetup';
const setupInput = {
...
describe('applyTableResetSetup', () => {
    ...
    it('starts play ledger with a table reset note only', () => {
        ...
        const reset = applyTableResetSetup(state, setupInput, state.tableMeta.ownerPersonId);
        const resetEntries = reset.ledger.entries.filter((e) => e.description.includes(TABLE_RESET_LEDGER_MESSAGE));
