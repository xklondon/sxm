...
import type { AddPlayerInput } from './session';
...
export function createNewBlackjackTable(): GameState {
  const id = generateId();
...
export function startNewTable(state: GameState): GameState {
  ...
  for (const personId of listPersonBankrollOwnerIds(next)) {
    ...
    if (
      bankId &&
      personsShareOneChipPot(next, personId, bankId) &&
      derivePlayerBalanceFromLedger(bankId, next.ledger) > 0
    ) {
      continue;
    }
    if (seatChips > 0) {
...
export function confirmTableAgreement(
  state: GameState,
  stakeDescription: string,
  startingChipsEachSeat = DEFAULT_TABLE_CHIPS,
...
export function recordTableOutcome(
  state: GameState,
  winnerId: string | null,
  loserId: string | null,
): GameState {
  const stake =
    state.tableMeta.agreement?.stakeDescription ?? 'Agreed stake';
  const outcome = {
    winnerId,
    loserId,
...
export function switchGameType(state: GameState, gameType: import('../../types').GameType): GameState {
...
export function selectTableGame(state: GameState, gameType: import('../../types').GameType): GameState {
...
export function nextBoxDisplayName(state: GameState): string {
...
export function addSeatAtTable(
  state: GameState,
  input: AddPlayerInput,
): GameState {
  return mergeSessionUpdate(
    state,
    addPlayer(state.session, state.players, state.ledger, input),
  );
}

export function removeSeatFromTable(state: GameState, boxId: string): GameState {
  ...
  const boxSlots =
    slotNum !== undefined
      ? state.tableMeta.boxSlots.map((s) =>
          s.slotNumber === slotNum ? { ...s, playerId: null, passiveNames: [] } : s,
        )
      : state.tableMeta.boxSlots;
  const nextSelected =
    state.selectedSeatId === boxId ? null : state.selectedSeatId;
  return {
...
export function defaultBlackjackSeatId(state: GameState): string | null {
