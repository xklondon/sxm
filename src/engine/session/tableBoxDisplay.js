...
export function getStakerPersonIdsForBox(state, boxPlayerId) {
    const stake = state.tableMeta.boxStakes[boxPlayerId];
    if (!stake || stake.amount <= 0) {
        return [];
    }
    const ids = new Set(stake.stakerPersonIds ?? []);
 ...
 */
export function getRunningBoxSlotsForPerson(state, personId) {
...
export function getCoBoxSlotsForPerson(state, personId) {
    const co = [];
    for (const slot of state.tableMeta.boxSlots) {
        if (!slot.playerId) {
            continue;
