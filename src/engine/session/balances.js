...
import { getAvailableChipsForBankrollOwner, getBoxSlotNumbersForBankrollOwner, getLedgerBalanceForBankrollOwner, getTotalBettingExposureForBankrollOwner, listBankrollParticipantIds, logAccountsPanelPeopleBalances, resolveBankrollOwnerIdForBox, } from './bankroll';
/** @deprecated Use bankroll owner helpers — box positions do not hold ledger balances. */
export function getLedgerBalance(state, participantId) {
...
export function getOpenTableStake(state, boxPlayerId) {
...
export function getConfirmedBetTotal(state, boxPlayerId) {
    if (boxPlayerId === state.session.bankPlayerId) {
        return 0;
    }
    ...
    return listHandKeysForPlayer(round.playerHands, boxPlayerId).reduce((sum, key) => sum + (round.playerHands[key]?.currentBet ?? 0), 0);
}
/** Spendable chips for a person/bank bankroll minus all box exposure. */
export function getAvailableChips(state, bankrollOwnerId) {
    return getAvailableChipsForBankrollOwner(state, bankrollOwnerId);
}
export function buildAccountRows(state) {
    const rows = [];
    ...
    logAccountsPanelPeopleBalances(rows.map((r) => ({
        label: r.label,
