...
import { finalizeInviteJoinAtTable, formatPlayerJoinedMessage, clearTableUiEphemeral, } from './inviteJoin';
...
describe('invite join allocation', () => {
    it('allocates starting chips and assigns next free box', () => {
        ...
        const slot = joined.state.tableMeta.boxSlots.find((s) => s.nativeAssignedPersonId === guestId);
        ...
        expect(joined.state.tableMeta.tableNotice?.message).toBe(formatPlayerJoinedMessage('Guest', joined.state.tableMeta.joinHighlight.slotNumber));
    ...
    it('joined player can place minimum bet immediately', () => {
        ...
        const boxId = joined.state.tableMeta.boxSlots.find((s) => s.nativeAssignedPersonId === guestId).playerId;
