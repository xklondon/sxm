...
function participantLabel(state: GameState, participantId: string): string {
...
export function personsShareOneChipPot(
  state: GameState,
  idA: string,
  idB: string,
): boolean {
...
export function resolveCanonicalBankrollOwnerId(
  state: GameState,
...
export function listSharedPotBankrollOwnerIds(
  state: GameState,
  bankrollOwnerId: string,
): string[] {
  const bankId = state.session.bankPlayerId;
  ...
  const ids = new Set<string>([bankId, bankrollOwnerId]);
  const ownerId = state.tableMeta.ownerPersonId;
  if (ownerId) {
    ids.add(ownerId);
  }
...
export function getSharedPotLedgerBalance(
  state: GameState,
  bankrollOwnerId: string,
): number {
...
export function getSharedPotBettingExposure(
  state: GameState,
  bankrollOwnerId: string,
): number {
...
export function getSharedPotAvailableChips(
  state: GameState,
  bankrollOwnerId: string,
): number {
  return (
    getSharedPotLedgerBalance(state, bankrollOwnerId) -
    getSharedPotBettingExposure(state, bankrollOwnerId)
  );
}

/** True when this participant uses the bank ledger as the single chip pot. */
export function usesSharedBankPlayerPot(
  state: GameState,
  bankrollOwnerId: string,
): boolean {
  const bankId = state.session.bankPlayerId;
