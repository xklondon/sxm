...
export function getTotalOpenStakesForBankrollOwner(state, bankrollOwnerId) {
    ...
    return getBoxIdsForBankrollOwner(state, bankrollOwnerId).reduce((sum, boxId) => sum + getStakeForBox(state, boxId), 0);
}
...
export function getTotalInRoundBetsForBankrollOwner(state, bankrollOwnerId) {
    ...
    return getBoxIdsForBankrollOwner(state, bankrollOwnerId).reduce((sum, boxId) => {
        return (sum +
            listHandKeysForPlayer(round.playerHands, boxId).reduce((handSum, key) => handSum + (round.playerHands[key]?.currentBet ?? 0), 0));
    }, 0);
}
