...
export function createInitialZilchState(playerIds, settings) {
...
export function startTurn(state, playerId) {
    ...
    const phase = state.phase === 'final-round' ? 'final-round' : 'player-turn';
    return {
        ...state,
        phase,
...
function diceToRollCount(state) {
...
export function rollDice(state, settings, rng = Math.random, now = Date.now()) {
...
export function completeDiceRoll(state) {
    if (!state.diceAnimation.isRolling) {
        return state;
    }
    let dice = state.dice;
...
function findCombination(state, combinationId) {
    return state.availableCombinations.find((c) => c.id === combinationId);
}
function combinationDiceAvailable(state, combo) {
...
export function keepCombination(state, combinationId) {
    ...
    const nextDice = state.dice.map((d) => keptIds.has(d.id)
        ? { ...d, isKept: true, isAvailable: false, keptGroupId: groupId }
        : d);
    const remainingAvailable = nextDice.filter((d) => d.isAvailable && !d.isKept);
    const nextCombinations = detectZilchCombinations(nextDice).filter((c) => combinationDiceAvailable({ ...state, dice: nextDice }, c));
...
export function endTurnWithZilch(state) {
    const playerId = state.currentPlayerId;
    if (!playerId) {
        throw new Error('No active player');
    }
...
function incrementRoundsPlayed(state, playerId) {
    return {
        ...state,
        players: state.players.map((p) => p.playerId === playerId
...
export function advanceToNextPlayer(state) {
    ...
    if (state.phase === 'final-round' && state.playersRemainingFinalTurn) {
        ...
        return startTurn({ ...state, playersRemainingFinalTurn: remaining }, nextPlayerId);
    }
...
function highestScorePlayerId(state) {
    let best = null;
...
function allPlayersReachedRoundLimit(state) {
    const limit = state.roundLimit ?? 0;
    return state.players.every((p) => p.roundsPlayed >= limit);
}
export function checkZilchGameEnd(state) {
...
export function getZilchWinnerId(state) {
    if (state.winnerPlayerId) {
        return state.winnerPlayerId;
    }
    return highestScorePlayerId(state);
}
export function canRollDice(state) {
...
export function canBank(state) {
    ...
    return (state.turnScore > 0 &&
        state.keptThisRoll &&
        (state.phase === 'player-turn' || state.phase === 'awaiting-keep-selection'));
}
export function canKeepCombination(state) {
...
export function quitTurn(state) {
