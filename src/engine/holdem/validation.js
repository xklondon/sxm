...
export function canCheckHoldem(round, playerId) {
...
export function canCallHoldem(ledger, round, playerId) {
    if (round.activePlayerId !== playerId) {
        return false;
    }
    const ps = round.playerStates[playerId];
...
export function canBetHoldem(ledger, round, playerId, amount) {
...
export function canRaiseHoldem(ledger, round, playerId, targetTotal) {
    if (round.activePlayerId !== playerId) {
        return false;
    }
    const ps = round.playerStates[playerId];
...
export function canFoldHoldem(round, playerId) {
    return round.activePlayerId === playerId &&
        round.playerStates[playerId]?.actionStatus !== 'folded';
}
export function hasEnoughCardsForHoldemStart(deck, playerCount) {
    if (!deck) {
        return false;
    }
...
export function runHoldemEngineChecks() {
    const results = [];
    try {
        assertMinHoldemPlayers({ playerIds: ['a'] });
