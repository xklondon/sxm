...
export function validateHoldemAction(ctx) {
...
function commitChips(ctx, entryType, chipAmount, description) {
    if (chipAmount > 0) {
    ...
    const ledgerResult = appendHoldemLedgerEntry(ctx.session, ctx.ledger, ctx.playerId, entryType, -chipAmount, description);
...
function resetOthersActed(round, exceptPlayerId) {
    const playerStates = {};
...
export function checkHoldemPlayer(ctx) {
    const ps = ctx.round.playerStates[ctx.playerId];
...
export function betHoldemPlayer(ctx) {
    ...
    const result = commitChips(ctx, 'bet-placed', amount, `Bet ${amount} chips`);
    return {
        ...result,
...
export function callHoldemPlayer(ctx) {
    const ps = ctx.round.playerStates[ctx.playerId];
    const toCall = ctx.round.currentBet - ps.playerBetsThisStreet;
    if (toCall <= 0) {
        throw new Error('Nothing to call — check instead');
    }
    return commitChips(ctx, 'call-placed', toCall, `Call ${toCall} chips`);
}
export function raiseHoldemPlayer(ctx) {
    ...
    const result = commitChips(ctx, 'bet-increased', additional, `Raise to ${targetTotal} chips`);
    return {
        ...result,
        round: resetOthersActed({
            ...result.round,
...
export function foldHoldemPlayer(ctx) {
    const ps = ctx.round.playerStates[ctx.playerId];
    const ledgerResult = appendHoldemLedgerEntry(ctx.session, ctx.ledger, ctx.playerId, 'fold-recorded', 0, `${ctx.playerId} folds`);
    const round = appendActionLog({
        ...ctx.round,
        playerStates: {
            ...ctx.round.playerStates,
...
export function postBlind(ctx, blindType, amount) {
    const label = blindType === 'small' ? 'Small blind' : 'Big blind';
    const balance = derivePlayerBalanceFromLedger(ctx.playerId, ctx.ledger);
    if (amount > balance) {
        throw new Error(`${label}: not enough chips (balance ${balance})`);
    }
    const ledgerResult = appendHoldemLedgerEntry(ctx.session, ctx.ledger, ctx.playerId, 'blind-posted', -amount, `${label}: ${amount} chips`);
    const ps = ctx.round.playerStates[ctx.playerId];
    const updatedPs = {
        ...ps,
