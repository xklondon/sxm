...
export interface RankedHoldemHand {
...
function rankFiveCards(cards: Card[]): RankedHoldemHand {
...
export function rankHoldemHand(cards: Card[]): RankedHoldemHand {
  if (cards.length !== 5) {
...
export function evaluateBestHoldemHand(
  holeCards: Card[],
  communityCards: Card[],
): RankedHoldemHand {
  ...
  let best: RankedHoldemHand | null = null;
...
export function compareHoldemHands(
  handA: RankedHoldemHand,
  handB: RankedHoldemHand,
): number {
...
export function runHandEvaluatorChecks(): { passed: boolean; results: string[] } {
  const results: string[] = [];

  const c = (id: string, rank: Rank, suit: Card['suit']): Card => ({ id, rank, suit });

  const royal = evaluateBestHoldemHand(
    [c('AH', 'A', 'hearts'), c('KH', 'K', 'hearts')],
    [c('QH', 'Q', 'hearts'), c('JH', 'J', 'hearts'), c('10H', '10', 'hearts')],
  );
  results.push(royal.category === 'royal-flush' ? 'pass: royal flush' : 'fail: royal flush');

  const pair = evaluateBestHoldemHand(
    [c('AS', 'A', 'spades'), c('AD', 'A', 'diamonds')],
    [c('2C', '2', 'clubs'), c('7H', '7', 'hearts'), c('9S', '9', 'spades')],
  );
  results.push(pair.category === 'two-pair' || pair.category === 'one-pair'
    ? 'pass: pair detection'
    : 'fail: pair detection');

  const cmp = compareHoldemHands(
    evaluateBestHoldemHand([c('AS', 'A', 'spades'), c('KS', 'K', 'spades')], [c('QS', 'Q', 'spades'), c('JS', 'J', 'spades'), c('9S', '9', 'spades')]),
    evaluateBestHoldemHand([c('2D', '2', 'diamonds'), c('3D', '3', 'diamonds')], [c('4D', '4', 'diamonds'), c('5D', '5', 'diamonds'), c('7H', '7', 'hearts')]),
  );
