...
import { getCallerPersonIdForBox, getAssignedSlotForPerson, syncPlayerOrderAndAssignments, ensureBoxPositionForPerson, } from '../session/playerAssignment';
...
import { actingRound, boxPlayerId, findCardId, tableAfterStartPlaying, } from './sanity/fixtures';
...
describe('box ownership / allocation rules', () => {
    ...
    it('first bettor on free Box 3 becomes caller; second bettor does not', () => {
        ...
        const slots = state.tableMeta.boxSlots.map((s) => s.playerId === box3 ? { ...s, nativeAssignedPersonId: null } : s);
    ...
    it('next betting round clears free-box caller on slot and stake', () => {
        ...
        state = claimBoxSlot(state, 3);
        const box3 = boxPlayerId(state, 3);
        const slots = state.tableMeta.boxSlots.map((s) => s.playerId === box3 ? { ...s, nativeAssignedPersonId: null, callerPersonId: 'stale' } : s);
        state = addChipToBoxStake({ ...state, tableMeta: { ...state.tableMeta, boxSlots: slots } }, box3, 10, p1);
        state = {
            ...state,
    ...
    it('new joiner assigned Box 3 wins caller over prior-round first bettor', () => {
        ...
        state = claimBoxSlot(state, 3);
        const box3 = boxPlayerId(state, 3);
        const slots = state.tableMeta.boxSlots.map((s) => s.playerId === box3
