...
import { getNativeAssignedPersonForSlot, resolveControllerPersonId, } from '../session/playerAssignment';
import { formatInsufficientChipsMessage } from './playFlow';
import { getBlackjackProtocolForState } from './protocolState';
import { formatMinBetMultipleMessage, isBetValidUnderProtocol, } from './protocols/activeRules';
function occupiedBoxIds(state) {
    return state.tableMeta.boxSlots
        .map((s) => s.playerId)
        .filter((id) => id !== null);
}
function slotForBox(state, boxPlayerId) {
...
export function getStakeForBox(state, boxPlayerId) {
    return state.tableMeta.boxStakes[boxPlayerId]?.amount ?? 0;
}
export function getStakeChipsForBox(state, boxPlayerId) {
    return (state.tableMeta.boxStakes[boxPlayerId]?.chips ?? []);
}
export function getBoxesWithStakes(state) {
    const ids = new Set();
...
export function hasAnyStakes(state) {
    return getBoxesWithStakes(state).length > 0;
}
export function isBettingOpen(state) {
    return !state.tableMeta.bettingLocked;
}
function resolveStakerPersonId(state, boxPlayerId, stakerPersonId) {
    if (stakerPersonId) {
...
function resolveCallerForStake(state, boxPlayerId, stakerPersonId, existing) {
...
export function addChipToBoxStake(state, boxPlayerId, chip, stakerPersonId) {
    ...
    const entry = state.tableMeta.boxStakes[boxPlayerId] ?? { amount: 0, chips: [] };
...
export function confirmBoxStake(state, boxPlayerId) {
    if (state.tableMeta.bettingLocked) {
        throw new Error('Bets are locked for this round.');
    }
...
export function isBoxStakeConfirmed(state, boxPlayerId) {
    const entry = state.tableMeta.boxStakes[boxPlayerId];
...
export function getStakeBetValidationMessage(state, boxPlayerId) {
    const entry = state.tableMeta.boxStakes[boxPlayerId];
    const amount = entry?.amount ?? 0;
    if (amount <= 0) {
        return null;
...
export function removeLastChipFromBoxStake(state, boxPlayerId) {
    if (state.tableMeta.bettingLocked) {
        throw new Error('Bets are locked for this round.');
    }
    const entry = state.tableMeta.boxStakes[boxPlayerId];
...
export function clearBoxStake(state, boxPlayerId) {
    if (state.tableMeta.bettingLocked) {
        throw new Error('Bets are locked for this round.');
    }
    const entry = state.tableMeta.boxStakes[boxPlayerId];
...
export function unlockBettingForNextRound(state) {
    return {
        ...state,
        tableMeta: {
            ...state.tableMeta,
