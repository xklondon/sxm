...
import { applyCardVisibility, cardRevealScopeKey, countVisibleCards, maxVisibilityForRound, shouldHydrateCardRevealScope, shouldUseOrderedInitialReveal, totalCardCount, } from './cardRevealDisplay';
...
describe('card reveal hydration helpers', () => {
    ...
    it('ordered initial reveal uses canonical plan for any pending initial-deal cards', () => {
        ...
        expect(shouldUseOrderedInitialReveal('initial-deal', empty, { dealer: 2, hands: { 'p:0': 2, 'p2:0': 2 } })).toBe(true);
        expect(shouldUseOrderedInitialReveal('player-turns', empty, { dealer: 2, hands: { 'p:0': 2 } })).toBe(true);
        expect(shouldUseOrderedInitialReveal('bank-turn', { dealer: 1, hands: {} }, { dealer: 3, hands: {} })).toBe(false);
    });
    ...
    it('bank-turn hydrate shows all existing dealer cards without replay steps', () => {
        ...
        state = { ...state, blackjack: round };
        const target = maxVisibilityForRound(round);
        expect(shouldUseOrderedInitialReveal('bank-turn', { dealer: 0, hands: {} }, target)).toBe(false);
        const hydrated = applyCardVisibility(state, target);
