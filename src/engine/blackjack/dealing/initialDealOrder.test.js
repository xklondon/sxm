...
import { buildInitialDealPlan, buildInitialDealPlanFromHandKeys, dealNextInitialCard, } from '../initialDeal';
import { buildInitialDealSequence } from '../dealSequence';
import { buildInitialRevealSteps, shouldUseOrderedInitialReveal, shouldHydrateCardRevealScope, applyCardVisibility, maxVisibilityForRound, } from './cardRevealDisplay';
...
import { shuffleToStartOnState, beginInitialDealOnState, dealNextInitialCardOnState, dealCardsButtonOnState, completeStepwiseInitialDealIfNeeded, } from '../gameState';
function planLabels(handKeys, holeCardDealtLast = false) {
    return buildInitialDealPlanFromHandKeys(handKeys, holeCardDealtLast).map((step) => step.type === 'dealer'
        ? `D${step.cardIndex}`
...
describe('initial deal order', () => {
    ...
    it('dealer up-card only after every box receives first card', () => {
        ...
        expect(plan.slice(0, firstDealerIdx).every((s) => s.type === 'box' && s.cardIndex === 0)).toBe(true);
    });
    ...
    it('deal sequence UI order matches engine plan for three boxes', () => {
        ...
        expect(uiSteps.map((s) => (s.type === 'dealer' ? `D${s.cardIndex}` : `${s.handKey}:${s.cardIndex}`))).toEqual(engineSteps.map((s) => (s.type === 'dealer' ? `D${s.cardIndex}` : `${s.handKey}:${s.cardIndex}`)));
    });
    it('ordered reveal applies when player-turns state arrives before UI catch-up', () => {
        expect(shouldUseOrderedInitialReveal('player-turns', { dealer: 0, hands: {} }, { dealer: 2, hands: { 'p:0': 2 } })).toBe(true);
        expect(shouldUseOrderedInitialReveal('bank-turn', { dealer: 0, hands: {} }, { dealer: 3, hands: {} })).toBe(false);
    });
    ...
    it('stepwise deal gives every box a first card before any box receives a second', () => {
        ...
        const counts = Object.fromEntries(handKeys.map((k) => [k, 0]));
        let guard = 0;
        while (state.blackjack?.status === 'initial-deal' && guard < 20) {
            guard += 1;
            const result = dealNextInitialCard(state.session, state.players, state.deck, state.blackjack, state.blackjackSettings);
