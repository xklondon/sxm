...
import { dealCardsButtonOnState, doubleDownBlackjackOnState, hitBlackjackOnState, shuffleToStartOnState, startBlackjackRound, completeStepwiseInitialDealIfNeeded, } from './gameState';
...
describe('Las Vegas protocol round', () => {
    it('double on hard 9/10/11 deals one card and stands', () => {
        for (const ranks of [['5', '4'], ['5', '5'], ['5', '6']]) {
    ...
    it('doubleDownBlackjackPlayer draws exactly one card', () => {
        ...
        const result = doubleDownBlackjackPlayer(state.session, state.players, state.ledger, deck, round, handKey, bankrollContextFromState(state), state.blackjackSettings, getBlackjackProtocolForState(state));
