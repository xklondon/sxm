...
import { takeInsuranceOnState, declineInsuranceOnState, applyInsuranceAdvanceOnState, } from './gameState';
...
import { getInsuranceActionsForController, getPendingInsurancePlayerIds, } from '../../components/blackjackViewPhase';
import { getInsuranceEligibleBoxIds } from './protocols/activeRules';
import { allInsuranceDecisionsResolved, isInsuranceBoxDecisionResolved, } from './insurance';
import { LAS_VEGAS_PROTOCOL } from './protocols';
import { tableWithClaimedBox, tableWithTwoBoxesSamePerson, boxPlayerId, findCardId, } from './sanity/fixtures';
...
function insuranceRoundTwoBoxes(state, box1, box2, bets) {
...
function readyTwoBoxInsurance(ownerName = 'Alice') {
...
describe('multi-box insurance', () => {
    ...
    it('auto-skips unfunded boxes so the phase does not stall', () => {
        ...
        void personId;
        expect(isInsuranceBoxDecisionResolved(state, state.blackjack, LAS_VEGAS_PROTOCOL, box1)).toBe(true);
        expect(isInsuranceBoxDecisionResolved(state, state.blackjack, LAS_VEGAS_PROTOCOL, box2)).toBe(true);
