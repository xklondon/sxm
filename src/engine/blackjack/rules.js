...
import { LAS_VEGAS_PROTOCOL, getProtocolDisplayRules, isSettingsMatchingProtocol, } from './protocols';
...
export function isLasVegasDefaultSettings(settings) {
    return isSettingsMatchingProtocol(settings, LAS_VEGAS_PROTOCOL);
}
export function insuranceBetMax(mainBet) {
    return Math.floor(mainBet / 2);
}
export function insuranceWinPayout(insuranceBet) {
    return insuranceBet + insuranceBet * INSURANCE_PAYOUT_RATIO;
}
export function runBlackjackRulesAudit(settings = DEFAULT_BLACKJACK_SETTINGS) {
    const results = [];
    ...
    results.push({
        ...
        passed: settings.numberOfDecks >= protocol.shoe.minDecks &&
