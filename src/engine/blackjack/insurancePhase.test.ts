...
describe('insurance phase', () => {
  ...
  it('take and decline advance when all eligible boxes decided', () => {
    ...
    declineState = {
      ...
      blackjack: insuranceRound(
        declineState,
        declineBoxId,
        [findCardId(declineState.deck!, '10'), findCardId(declineState.deck!, '8')],
        50,
      ),
    };
  ...
  it('auto-advances when no eligible insurance decisions remain', () => {
    ...
    const round = insuranceRound(
      ...
      [findCardId(state.deck!, '10'), findCardId(state.deck!, '9')],
      50,
      { insuranceDeclined: { [boxId]: true }, insuranceBets: {} },
    );
    state = { ...state, blackjack: round };
  ...
  it('double and split respect protocol and bankroll on legal hands', () => {
    ...
    state = {
      ...
      blackjack: actingRound(
        state,
        boxId,
        [findCardId(state.deck!, '8'), findCardId(state.deck!, '8')],
        25,
      ),
    };
