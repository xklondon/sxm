...
export function runDealingSanityChecks() {
    const results = [];
    results.push(check('RTL turn order: slot 1 before slot 2', getBettingPlayerIds({
    ...
    const labels = plan.map((step) => step.type === 'dealer'
        ? `dealer:${step.cardIndex}`
        : `${step.handKey}:${step.cardIndex}`);
    results.push(check('initial deal: all box first cards before dealer up-card', labels[0] === 'boxSlot2:0:0' &&
        labels[1] === 'boxSlot1:0:0' &&
        labels[2] === 'dealer:0', labels.join(' → ')));
    results.push(check('initial deal: second pass box then dealer hole', labels[3] === 'boxSlot2:0:1' &&
        labels[4] === 'boxSlot1:0:1' &&
        labels[5] === 'dealer:1', labels.join(' → ')));
    ...
    results.push(check('natural deal sequence matches initial deal plan length', natural.length === plan.length));
    results.push(check('natural deal sequence is one card per step', natural.every((s) => s.stepIndex >= 0 && s.label.length > 0)));
