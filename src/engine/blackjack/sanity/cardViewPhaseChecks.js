...
import { addChipToBoxStake, confirmBoxStake, getStakeForBox, removeLastChipFromBoxStake, } from '../stakes';
...
import { actionButtonsUseTappableClass, getCardViewHeroBoxId, getCardViewHeroHandKey, getPendingInsurancePlayerIds, heroActionsRenderBeforeCards, isBettingPhase, isCardViewMiniBox, isInsurancePhase, isPlayerTurnPhase, miniBoxesRenderAfterHeroCards, showBettingFooter, showBettingMainStage, showDealerHeader, showEvenMoneyControls, showInsuranceControls, showPlayerActionControls, canShowPlayerDecisionControls, showStitchedPlayerCards, hitStandUseActionBarButtonClass, } from '../../../components/blackjackViewPhase';
...
export function runCardViewPhaseChecks() {
    const results = [];
    ...
    results.push(check('no stitched actions in betting', !canShowPlayerDecisionControls(state, 'betting', { cardRevealComplete: true })));
    ...
    results.push(check('card view top-chip retraction restores available', getStakeForBox(state, boxId) === 10 &&
    ...
    results.push(check('player action controls in player phase', showPlayerActionControls('player', playerRound)));
    results.push(check('no player controls during insurance phase', !showPlayerActionControls('insurance', { ...playerRound, insuranceOfferPending: true })));
    const evenMoneyRound = { ...playerRound, evenMoneyOfferHandKey: `${boxId}:0`, activeHandKey: `${boxId}:0` };
    results.push(check('even-money controls replace player actions', showEvenMoneyControls('player', evenMoneyRound)));
    results.push(check('even-money hides hit/stand controls', showEvenMoneyControls('player', evenMoneyRound) &&
    ...
    results.push(check('insurance controls visible in insurance phase', showInsuranceControls('insurance', insRound)));
    results.push(check('dealer ace waits for all insurance before bank', !allInsuranceResolved(insState, insRound, LAS_VEGAS_PROTOCOL) &&
        applySkipBankIfNeeded(insState.session, insRound).status === 'player-turns'));
    results.push(check('pending insurance lists unresolved eligible boxes', getPendingInsurancePlayerIds(insState, insRound).includes(insB2)));
    ...
    results.push(check('card view double respects activeRules', canDoubleBlackjackForState(state, `${boxPlayerId(state, 1)}:0`)));
    ...
    results.push(check('ended game personal ledger add', loadScoreLedgerEntries().some((e) => e.tableId === endState.session.id)));
    ...
    results.push(check('betting hero is selected seat', getCardViewHeroBoxId('betting', null, box1, box2) === box1));
    results.push(check('player hero follows active turn box', getCardViewHeroBoxId('player', box2, box1, box1) === box2));
    results.push(check('turn advance switches hero box', getCardViewHeroBoxId('player', box1, box1, box1) === box1 &&
        getCardViewHeroBoxId('player', box2, box1, box1) === box2));
    results.push(check('hero hand key tracks active hand on turn box', getCardViewHeroHandKey('player', { ...actingRound(twoBoxState, box2, [findCardId(twoBoxState.deck, '10'), findCardId(twoBoxState.deck, '9')], 25), activeHandKey: `${box2}:0` }, box1) === `${box2}:0`));
    results.push(check('non-current box is mini representation', isCardViewMiniBox(box1, box2) && !isCardViewMiniBox(box2, box2)));
    results.push(check('shared dealer actions layout anchor', BLACKJACK_TABLE_LAYOUT.dealerActions === 'dealer-block__actions'));
    results.push(check('shared accounts panel layout anchor', BLACKJACK_TABLE_LAYOUT.accountsPanel === 'bj-accounts-panel'));
    results.push(check('shared chip tray layout anchor', BLACKJACK_TABLE_LAYOUT.chipTrayWrap === 'bj-casino__tray-wrap'));
    results.push(check('playing phase total/actions render before hero cards', heroActionsRenderBeforeCards()));
    results.push(check('playing phase mini boxes render after hero cards', miniBoxesRenderAfterHeroCards()));
