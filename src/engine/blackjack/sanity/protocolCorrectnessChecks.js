...
import { addChipToBoxStake, clearBoxStake, confirmBoxStake, getStakeForBox, removeLastChipFromBoxStake, } from '../stakes';
...
import { buildActiveRulesHandContext, canDoubleUnderProtocol, canSplitUnderProtocol, isBetValidUnderProtocol, shouldOfferEvenMoney, shouldPayNaturalImmediately, } from '../protocols/activeRules';
...
export function runProtocolCorrectnessSanityChecks() {
    const results = [];
    results.push(check('activeRules validates min bet multiples', !isBetValidUnderProtocol(LAS_VEGAS_PROTOCOL, 7, 5).valid &&
    ...
    results.push(check('default play flow is auto-18', getPlayFlowForPerson(state, personId) === 'auto-18'));
    ...
    results.push(check('bet retraction restores available', getAvailableChipsForBankrollOwner(state, personId) === availableBefore));
    ...
    results.push(check('hard 11 double via activeRules state', canDoubleBlackjackForState(state, `${hardBox}:0`)));
    ...
    const splitCtx = buildActiveRulesHandContext(LAS_VEGAS_PROTOCOL, state.ledger, splitRound, `${hardBox}:0`, deck, splitOwnerId, getAvailableChipsForBankrollOwner(state, splitOwnerId));
    results.push(check('J/J split allowed under Las Vegas protocol', splitCtx !== null && canSplitUnderProtocol(LAS_VEGAS_PROTOCOL, splitHand, { ...splitCtx, deck })));
    state = setBlackjackProtocolOnState(state, EUROPEAN_SHOE_PROTOCOL.id, 'Alice');
    state = {
        ...state,
        blackjack: actingRound(state, hardBox, [five, six], 25),
    };
    results.push(check('hard 11 double under European protocol', canDoubleBlackjackForState(state, `${hardBox}:0`)));
    state = tableWithClaimedBox(1);
    ...
    results.push(check('bust keeps cards visible after hit', bustHand?.actionStatus === 'busted' && (bustHand?.cardIds.length ?? 0) >= 3));
    ...
    results.push(check('natural vs 6 pays immediately', shouldPayNaturalImmediately(LAS_VEGAS_PROTOCOL, [deck.cards.find((c) => c.id === ace), deck.cards.find((c) => c.id === king)], '6')));
    results.push(check('natural vs Ace offers even money path', shouldOfferEvenMoney(LAS_VEGAS_PROTOCOL, [deck.cards.find((c) => c.id === ace), deck.cards.find((c) => c.id === king)], 'A')));
    state = tableWithClaimedBox(1);
    ...
    results.push(check('auto-stand 18+ stands hand', autoResult.blackjack?.playerHands[blackjackHandKey(autoBox, 0)]?.actionStatus === 'stood'));
    ...
    const doubleCtx = buildActiveRulesHandContext(LAS_VEGAS_PROTOCOL, state.ledger, hardRound, blackjackHandKey(autoBox, 0), deck, autoOwnerId, getAvailableChipsForBankrollOwner(state, autoOwnerId));
    results.push(check('double rule source is activeRules adapter', doubleCtx !== null &&
    ...
    results.push(check('$500 wager label separate from chip count', getTableWagerDisplay(wagerState) === '$500' && wagerState.tableMeta.agreement?.defaultChips === 500));
    ...
    results.push(check('dealer ace waits for all insurance decisions', !allInsuranceResolved(insState, insRound, LAS_VEGAS_PROTOCOL)));
    results.push(check('insurance pending blocks bank draw advance', applySkipBankIfNeeded(insState.session, insRound).status === 'player-turns' &&
    ...
    results.push(check('all active boxes insurance resolved', allInsuranceResolved(insState, insResolvedRound, LAS_VEGAS_PROTOCOL)));
    ...
    results.push(check('invite origin uses LAN host not localhost', resolveTableInviteOrigin('192.168.0.56:5137', null) === 'http://192.168.0.56:5137'));
