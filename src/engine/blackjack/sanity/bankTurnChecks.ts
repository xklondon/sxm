...
function runBankDrawUntilStand(
  session: GameSession,
  players: Record<string, Player>,
  deck: Deck,
  round: BlackjackRound,
  settings: ReturnType<typeof protocolToBlackjackSettings>,
  protocol = LAS_VEGAS_PROTOCOL,
): { round: BlackjackRound; deck: Deck; draws: number } {
...
function cardRankValues(deck: Deck, ids: string[]): Card[] {
  return cardsFromIds(deck, ids.filter(Boolean));
}

export function runBankTurnSanityChecks(): SanitySuiteResult {
  const results = [];
  ...
  results.push(
    check(
      'dealer total 13 must draw (evaluateDealerDraw)',
      evaluateDealerDraw(cards13, vegasSettings, LAS_VEGAS_PROTOCOL).shouldDraw === true,
    ),
  );
  results.push(
    check(
      'dealer total 13 must draw (bankNeedsAnotherCard)',
      bankNeedsAnotherCard(cards13, vegasSettings, LAS_VEGAS_PROTOCOL) === true,
    ),
  );

  const drawn = runBankDrawUntilStand(
    session,
    players,
    deck,
    dealerRound(ids13),
  ...
  results.push(
    check(
      'dealer never stands on 13 — draws until 17+',
      finalValue >= 17 || finalValue > 21,
      `final=${finalValue} draws=${drawn.draws}`,
    ),
  );
  results.push(
    check(
      'dealer with 13 ends in banking or bank-turn only after 17+',
      drawn.round.status === 'banking' || drawn.round.status === 'bank-turn',
    ),
  );
  ...
  results.push(
    check('dealer total 16 must draw', shouldDealerDraw(cards16, vegasSettings, LAS_VEGAS_PROTOCOL)),
  );
  ...
  results.push(
    check(
      'dealer hard 17 must stand (S17)',
      shouldDealerDraw(cardsHard17, vegasSettings, LAS_VEGAS_PROTOCOL) === false,
    ),
  );
  ...
  results.push(
    check(
      'dealer soft 17 stands on S17 protocol',
      shouldDealerDraw(cardsSoft17, vegasSettings, LAS_VEGAS_PROTOCOL) === false,
    ),
  );
  ...
  results.push(
    check(
      'dealer soft 17 hits on H17 protocol',
      shouldDealerDraw(cardsSoft17, h17Settings, h17Protocol) === true,
    ),
  );

  const entered = enterBankingIfComplete(
    dealerRound(idsHard17),
    deck,
    vegasSettings,
    LAS_VEGAS_PROTOCOL,
  );
  results.push(
    check(
      'enterBankingIfComplete transitions hard 17 to banking',
      entered.status === 'banking',
    ),
  );

  results.push(
    check(
      'enterBankingIfComplete keeps bank-turn when dealer at 13',
      enterBankingIfComplete(dealerRound(ids13), deck, vegasSettings, LAS_VEGAS_PROTOCOL).status ===
        'bank-turn',
    ),
  );
  ...
  results.push(
    check(
      'European S17: dealer 13 draws',
      shouldDealerDraw(cards13, europeanSettings, EUROPEAN_SHOE_PROTOCOL),
    ),
  );
  ...
  results.push(
    check(
      'all busted boxes skip bank draw — zero dealer cards added',
      bustDraw.cardId === null &&
  ...
  const natRound = {
    ...
    outcomes: { [natHandKey]: 'win' as const },
  };
  const natDeckBefore = natTable.deck!.cards.length;
  const natDraw = drawSingleBankCard(
    natTable.session,
    natTable.players,
    natTable.deck!,
    natRound,
  ...
  results.push(
    check(
      'natural-blackjack-paid skips bank draw — zero dealer cards added',
      natDraw.cardId === null &&
