...
import { getAssignedSlotForPerson, getCallerPersonIdForBox, movePlayerInOrder, } from '../../session/playerAssignment';
...
function stake(state, boxId, amount) {
    return {
        ...state,
        tableMeta: {
            ...state.tableMeta,
...
export function runPhaseAssignmentSanityChecks() {
    const results = [];
    ...
    results.push(check('eligible box includes box 1', getEligibleDealBoxes(state).includes(box1), `eligible=${getEligibleDealBoxes(state).join(',')}`));
    ...
    results.push(check('deal deals to all five staked boxes', boxIds.every((id) => eligible.includes(id)) && eligible.length === 5, `eligible=${eligible.length}`));
    state = tableAfterStartPlaying(500);
    results.push(check('owner assigned to box 1', getAssignedSlotForPerson(state, state.tableMeta.ownerPersonId) === 1));
    state = tableAfterStartPlaying(500);
    ...
    results.push(check('second player assigned box 2', getAssignedSlotForPerson(state, bobId) === 2));
    state = movePlayerInOrder(state, bobId, 'up');
    results.push(check('reorder moves bob to box 1', getAssignedSlotForPerson(state, bobId) === 1 &&
        getAssignedSlotForPerson(state, orderOwnerId) === 2, `bob=${getAssignedSlotForPerson(state, bobId)} owner=${getAssignedSlotForPerson(state, orderOwnerId)}`));
    state = tableAfterStartPlaying(500);
    ...
    results.push(check('passive bettor is not caller on native box', getCallerPersonIdForBox(state, nativeBox) === passiveOwnerId, `caller=${getCallerPersonIdForBox(state, nativeBox)}`));
    state = tableAfterStartPlaying(500);
    ...
    const freeSlots = freeState.tableMeta.boxSlots.map((s) => s.slotNumber === freeSlot
    ...
    results.push(check('free box caller is first staking player', getCallerPersonIdForBox(freeState, freeBoxId) === freeOwnerId));
    state = tableAfterStartPlaying(500);
    ...
    state = shuffleToStartOnState(state);
    results.push(check('single player can call multiple boxes', getEligibleDealBoxes(state).length === 2));
