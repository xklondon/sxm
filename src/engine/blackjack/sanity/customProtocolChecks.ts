import {
  ...
  listCustomRuleReminders,
  resolveCustomProtocolToPreset,
} from '../../protocols/customProtocolBuilder';
...
export function runCustomProtocolSanityChecks(): SanitySuiteResult {
  ...
  results.push(
    check(
      'custom protocol resolves to preset with storage id',
      resolveCustomProtocolToPreset(custom).protocolId === storageId,
    ),
  );
  ...
  results.push(
    check(
      'social rule appears in custom reminders',
      reminders.some((r) => r.includes('Two queens') || r.includes('Social rule')),
    ),
  );
  ...
  results.push(
    check(
      'custom social rule surfaces in protocol message',
      msg.includes('Social rule') ||
