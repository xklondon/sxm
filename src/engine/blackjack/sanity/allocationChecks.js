...
import { findPersonPlayerIdByController, getAvailableChipsForBankrollOwner, getLedgerBalanceForBankrollOwner, getTotalBettingExposureForBankrollOwner, listPersonBankrollOwnerIds, } from '../../session/bankroll';
import { check } from './types';
import { baseTestTable, boxPlayerId, tableWithClaimedBox, tableWithTwoBoxesSamePerson, } from './fixtures';
...
function isBoxPlayerId(state, playerId) {
    return state.tableMeta.boxSlots.some((s) => s.playerId === playerId);
}
export function runAllocationSanityChecks() {
    const results = [];
    let state = baseTestTable();
    const bankId = state.session.bankPlayerId;
    const bankBal = derivePlayerBalanceFromLedger(bankId, state.ledger);
    results.push(check('setup allocates bank once', bankBal === 500, `bank ledger=${bankBal}`));
    ...
    if (personId) {
        ...
        results.push(check('owner bankroll allocated at setup; claim does not add chips', bal === 500, `person balance=${bal}`));
    }
    else {
    ...
    results.push(check('owner visible before box claim', setupPeople.some((p) => p.personId === setupOwnerId && p.available === 500)));
    state = tableWithClaimedBox(1);
    ...
    if (kId) {
        ...
        results.push(check('same person multiple boxes no extra allocation', bal === 500, `balance=${bal}`));
    }
    else {
    ...
    results.push(check('betting multiple boxes: exposure sums stakes', exposure === 100, `exposure=${exposure}`));
    results.push(check('betting multiple boxes: available is ledger minus exposure', available === 400, `available=${available}`));
    state = tableWithClaimedBox(1);
    state = claimBoxSlot(state, 2);
    state = claimBoxSlot(state, 3);
    ...
    results.push(check('assign chips to person once (not per box)', afterAssign - beforeAssign === 100 && afterAssign === 600, `before=${beforeAssign} after=${afterAssign}`));
    ...
    results.push(check('assign all persons once each', aliceAfter - aliceBefore === 100 &&
        annaAfter - annaBefore === 100 &&
        aliceAfter === 600 &&
        annaAfter === 600, `alice ${aliceBefore}->${aliceAfter} anna ${annaBefore}->${annaAfter}`));
    state = tableWithTwoBoxesSamePerson();
    const boxIds = state.tableMeta.boxSlots
        .map((s) => s.playerId)
        .filter((id) => id !== null);
    const badEntries = state.ledger.entries.filter((e) => (e.entryType === 'buy-in' || e.entryType === 'manual-adjustment') &&
        boxIds.includes(e.playerId));
    results.push(check('no box ledger allocations for buy-in/top-up', badEntries.length === 0, badEntries.length > 0 ? `offending=${badEntries.map((e) => e.playerId).join(',')}` : undefined));
    const personIds = listPersonBankrollOwnerIds(state);
    results.push(check('listPersonBankrollOwnerIds returns one id for multi-box person', personIds.length === 1, `count=${personIds.length}`));
    for (const boxId of boxIds) {
        results.push(check(`box ${boxId.slice(0, 6)} is not a bankroll target`, isBoxPlayerId(state, boxId) && !personIds.includes(boxId)));
    }
