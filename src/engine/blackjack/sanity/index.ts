...
import { mergeSuiteResults, type SanitySuiteResult } from './types';

export type { SanityCheckResult, SanitySuiteResult } from './types';
...
export function runBlackjackSanitySuite(): SanitySuiteResult {
  const legacy = runBlackjackEngineChecks();
  const legacySuite: SanitySuiteResult = {
  ...
  return mergeSuiteResults(
    legacySuite,
    runProtocolSanityChecks(),
    runDealingSanityChecks(),
    runBankrollSanityChecks(),
    runAllocationSanityChecks(),
    runSettlementSanityChecks(),
    runTableBalanceSanityChecks(),
    runOwnerSetupSanityChecks(),
    runPhaseAssignmentSanityChecks(),
    runGameplaySanityChecks(),
    runBankTurnSanityChecks(),
    runCustomProtocolSanityChecks(),
    runTablePeopleSanityChecks(),
    runGameplayUxSanityChecks(),
    runProtocolCorrectnessSanityChecks(),
    runCardViewPhaseChecks(),
  );
}

export function formatSanityFailures(result: SanitySuiteResult): string {
