...
export function runTablePeopleSanityChecks(): SanitySuiteResult {
  const results = [];
  ...
  results.push(
    check(
      'setup lists owner person row before box claim',
      buildTablePeopleRows(empty).some((r) => r.kind === 'person' && r.status === 'owner' && r.available === 500),
    ),
  );
  ...
  results.push(
    check(
      'person row lists box slots not separate box bankrolls',
      people.every((p) => p.kind === 'person' || p.kind === 'invite'),
    ),
  );
  results.push(
    check(
      'active person shows available chips',
      people.some((p) => p.kind === 'person' && p.available > 0),
    ),
  );
