...
export function runGameplaySanityChecks(): SanitySuiteResult {
  const results = [];
  ...
  const splitResult = splitBlackjackPlayer(
    state.session,
    state.players,
    state.ledger,
    deck,
  ...
  const doubleResult = doubleDownBlackjackPlayer(
    state.session,
    state.players,
    state.ledger,
    deck,
  ...
  results.push(
    check(
      'double adds exactly one card then stands',
      doubledHand.doubled === true &&
  ...
  const naturalRound: BlackjackRound = {
    ...state.blackjack!,
    status: 'banking',
  ...
  const vegasResolve = resolveBlackjackRound(
    state.session,
    state.players,
    state.ledger,
    deck,
  ...
  results.push(
    check(
      'natural blackjack pays 3:2 under Las Vegas protocol',
      vegasResolve.round.outcomes?.[`${boxId}:0`] === 'blackjack-win' &&
        (vegasResolve.round.resultMessages?.[`${boxId}:0`]?.includes('150') ?? false),
      vegasResolve.round.resultMessages?.[`${boxId}:0`],
    ),
  );

  const homeResolve = resolveBlackjackRound(
    state.session,
    state.players,
    state.ledger,
    deck,
  ...
  results.push(
    check(
      'natural blackjack pays 1:1 under Classic Home protocol',
      homeResolve.round.outcomes?.[`${boxId}:0`] === 'blackjack-win' &&
        (homeResolve.round.resultMessages?.[`${boxId}:0`]?.includes('200') ?? false),
      homeResolve.round.resultMessages?.[`${boxId}:0`],
    ),
  );
  ...
  results.push(
    check(
      'insurance offered when dealer Ace and protocol allows',
      shouldOfferInsurance(aceRound, aceDeck, vegasSettings),
    ),
  );
  results.push(
    check(
      'insurance not offered under European Shoe',
      !shouldOfferInsurance(
        aceRound,
        aceDeck,
        protocolToBlackjackSettings(EUROPEAN_SHOE_PROTOCOL),
      ),
    ),
  );
  ...
  results.push(
    check(
      ...
      dealerShowsAce(aceDeck, aceRound) && !dealerShowsAce(tenDeck, tenRound),
    ),
  );
...
function createInsuranceRound(boxId: string, deck: { cards: { id: string }[]; drawOrder: number[] }): BlackjackRound {
