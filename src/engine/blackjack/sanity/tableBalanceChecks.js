...
import { applyTableGameEndIfNeeded, evaluateTableGameEnd, isTableGameActive, } from '../../session/tableGameEnd';
...
export function runTableBalanceSanityChecks() {
    const results = [];
    ...
    results.push(check('player balance visible after allocation', Boolean(personRow?.showBalance && personRow.available === 500), `available=${personRow?.available}`));
    const bank = buildTableBankRow(claimed);
    results.push(check('bank row exists with name', Boolean(bank?.bankName)));
    results.push(check('bank row balance data', Boolean(bank && bank.balance === 500 && bank.bankName.length > 0), `balance=${bank?.balance}`));
    ...
    const lossRound = bankingRound(state, bId, [findCardId(deck, '10'), findCardId(deck, '9')], [findCardId(deck, 'K'), findCardId(deck, 'A')], 500);
    const resolved = resolveBlackjackRound(state.session, state.players, state.ledger, deck, lossRound, state.blackjackSettings, bankrollContextFromState(state));
    ...
    results.push(check('end condition when bank has all chips', state.tableMeta.gameStatus === 'ended' && endEval.winnerId === bankId, `status=${state.tableMeta.gameStatus}`));
    ...
    const winRound = bankingRound(state, winBoxId, [findCardId(deck, 'K'), findCardId(deck, '9')], [findCardId(deck, '10'), findCardId(deck, '8')], 500);
    ...
    results.push(check('end condition when player has all chips', state.tableMeta.gameStatus === 'ended' &&
        state.tableMeta.winnerId === winPersonId &&
        personBal >= 1000, `personBal=${personBal} winner=${state.tableMeta.winnerId}`));
    state = tableWithClaimedBox(1);
    ...
    results.push(check('no next round when game ended', state.tableMeta.gameStatus === 'ended' && !isTableGameActive(state)));
    let threw = false;
    try {
