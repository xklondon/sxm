...
export function runBankrollSanityChecks(): SanitySuiteResult {
  const results = [];
  ...
  if (personId) {
    ...
    results.push(
      check(
        'multi-box claim: single buy-in (500 chips once)',
        balAfterTwo === 500,
        `balance=${balAfterTwo}`,
      ),
    );
    ...
    results.push(
      check(
        'both boxes share same bankroll owner',
        Boolean(box1 && box2) &&
  ...
  if (boxId && ownerId) {
    ...
    results.push(
      check(
        'available chips reflect open bet (ledger minus in-round exposure)',
        getAvailableChipsForBankrollOwner(state, ownerId) === after - 100,
        `available=${getAvailableChipsForBankrollOwner(state, ownerId)} ledger=${after}`,
      ),
    );
  }
  ...
  results.push(
    check(
      'win pays stake + winnings to player bankroll',
      playerBalAfterWin - playerBalBeforeWin === 100,
      `delta=${playerBalAfterWin - playerBalBeforeWin}`,
    ),
  );
  ...
  if (state.session.bankPlayerId) {
    ...
    results.push(
      check(
        'bank bankroll tracked in ledger',
        typeof bankBal === 'number' && bankBal >= 0,
        `bankBalance=${bankBal}`,
      ),
    );
  }
