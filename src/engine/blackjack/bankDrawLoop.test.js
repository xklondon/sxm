...
describe('dealer bank draw loop', () => {
    ...
    it('draws sequentially until 17+ on hard totals', () => {
        ...
        while (round.status === 'bank-turn' && guard < 8) {
            guard += 1;
            const result = drawSingleBankCard(session, players, currentDeck, round, settings, LAS_VEGAS_PROTOCOL);
    ...
    it('drawSingleBankCard does not draw after dealer already stands', () => {
        ...
        const result = drawSingleBankCard(session, players, deck, round, settings, LAS_VEGAS_PROTOCOL);
