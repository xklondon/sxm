...
function shortResult(message: string): string {
...
export function getBoxComment(state: GameState, boxId: string): string {
    ...
    case 'resolved': {
      ...
      if (msg && hand && hand.currentBet > 0) {
        if (deck && hand.cardIds.length > 0) {
          const { value } = getBlackjackHandValue(cardsFromIds(deck, hand.cardIds));
          const dealerVal =
            deck && round.dealerCardIds.length > 0
...
export function getBankLabel(state: GameState): string {
