...
export function allPlayerHandsEliminated(session: GameSession, round: BlackjackRound): boolean {
...
export function hasActingHands(session: GameSession, round: BlackjackRound): boolean {
  return orderedHandKeys(session, round).some(
    (key) => round.playerHands[key]?.actionStatus === 'acting',
  );
}

/** Skip bank draw when every bet is already lost or fully settled. */
export function shouldSkipBankDraw(session: GameSession, round: BlackjackRound): boolean {
