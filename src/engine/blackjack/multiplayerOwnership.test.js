...
import { canDoubleUnderProtocol, buildActiveRulesHandContext, } from './protocols/activeRules';
...
import { getCallerPersonIdForBox, getAssignedSlotForPerson, isCallerForBox, ensureBoxPositionForPerson, } from '../session/playerAssignment';
...
import { actingRound, boxPlayerId, findCardId, tableAfterStartPlaying, tableWithClaimedBox, } from './sanity/fixtures';
function unassignedBoxWithStake(state, slotNumber, bettorId, amount) {
    let next = claimBoxSlot(state, slotNumber);
    const boxId = boxPlayerId(next, slotNumber);
    const slots = next.tableMeta.boxSlots.map((s) => s.slotNumber === slotNumber
...
describe('multiplayer box ownership', () => {
    ...
    it('unassigned box: decision owner resets at next betting round', () => {
        ...
        state = {
            ...
            tableMeta: {
                ...
                boxSlots: state.tableMeta.boxSlots.map((s) => s.playerId === boxId ? { ...s, callerPersonId: lockedCaller } : s),
            },
    ...
    it('invite join assigns next free box slot', () => {
        ...
        expect(state.tableMeta.boxSlots.find((s) => s.slotNumber === freeSlot.slotNumber)?.nativeAssignedPersonId).toBe(guestId);
    });
});
describe('Las Vegas double totals', () => {
    ...
    it('allows double after split only when split hand is two cards totaling 9/10/11', () => {
        ...
        const ctx = buildActiveRulesHandContext(LAS_VEGAS_PROTOCOL, state.ledger, round, handKey, deck, ownerId, getAvailableChipsForBankrollOwner(state, ownerId));
        ...
        const badCtx = buildActiveRulesHandContext(LAS_VEGAS_PROTOCOL, state.ledger, badRound, handKey, deck, ownerId, getAvailableChipsForBankrollOwner(state, ownerId));
        expect(canDoubleUnderProtocol(LAS_VEGAS_PROTOCOL, badRound.playerHands[handKey], badCtx)).toBe(false);
    });
});
