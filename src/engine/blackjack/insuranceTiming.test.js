...
import { canOfferInsuranceAfterInitialDeal, hasPlayerActionsStarted, isInitialDealRoundComplete, } from './initialDealGuards';
import { getBlackjackProtocolPhase } from './protocol';
import { beginInitialDealOnState, dealNextInitialCardOnState, lockBetsAndStartRoundOnState, shuffleToStartOnState, } from './gameState';
...
function hand(boxId, cardIds, bet = 50) {
    return {
...
function syntheticRound(state, partial) {
...
describe('insurance timing guards', () => {
    it('does not offer insurance during initial-deal status', () => {
        ...
        const activated = activateInsuranceOfferIfNeeded(begun.blackjack, begun.deck, begun.blackjackSettings, begun.session);
    ...
    it('offers insurance only after full initial deal with dealer Ace', () => {
        ...
        while (current.blackjack?.status === 'initial-deal' && guard < 20) {
            ...
            if (mid.blackjack.dealerCardIds.filter(Boolean).length === 1 &&
    ...
    it('no insurance when dealer upcard is not Ace after full deal', () => {
        ...
        const activated = activateInsuranceOfferIfNeeded(round, state.deck, state.blackjackSettings, state.session);
        expect(activated.insuranceOfferPending).not.toBe(true);
    });
