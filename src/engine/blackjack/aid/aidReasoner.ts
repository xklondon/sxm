...
import type { AidRecommendedAction } from '../protocols/types';
...
export interface AidStructuredAdvice {
  headline: string;
  protocolName: string;
  recommendedAction: AidRecommendedAction;
...
export interface AidReasonerInput {
...
function dealerUpValue(dealerRank: Rank): number {
  if (dealerRank === 'A') {
    return 11;
  }
...
function dealerPhrase(dealerRank: Rank): string {
  if (dealerRank === 'A') {
    return 'dealer Ace';
  }
  if (['K', 'Q', 'J', '10'].includes(dealerRank)) {
...
function pairDescription(c0: Card, c1: Card): string {
  if (c0.rank === c1.rank) {
...
function collectAvailableActionsWithDeck(
  round: BlackjackRound,
  handKey: string,
  _deck: Deck,
  protocol: BlackjackProtocol,
...
function strategyToRecommended(action: StrategyAction): AidRecommendedAction {
  switch (action) {
    case 'hit':
      return 'hit';
    case 'stand':
...
function buildRisk(action: StrategyAction, dealerRank: Rank, value: number): string {
  const d = dealerUpValue(dealerRank);
...
function buildFunLine(action: StrategyAction, cards: Card[]): string {
...
function reasonInsuranceAdvice(protocol: BlackjackProtocol): AidStructuredAdvice {
...
function reasonBlackjackAdvice(protocol: BlackjackProtocol): AidStructuredAdvice {
  return {
...
export function reasonAidAdvice(input: AidReasonerInput): AidStructuredAdvice | null {
...
export function formatAidStructuredAdvice(advice: AidStructuredAdvice): string {
  const parts = [
