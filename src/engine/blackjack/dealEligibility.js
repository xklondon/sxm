...
function getProtocolPhase(state) {
...
export function getTableMinimumBet(state) {
...
function isRoundInPlay(state) {
    const status = state.blackjack?.status;
    return Boolean(status && status !== 'betting' && status !== 'resolved');
}
function buildStakesByBox(state) {
 ...
 */
export function getEligibleDealBoxes(state) {
...
export function hasEligibleDealBoxes(state) {
    return getEligibleDealBoxes(state).length > 0;
}
export function isStakeBelowMinimum(state, boxPlayerId) {
    const stake = getStakeForBox(state, boxPlayerId);
    if (stake <= 0) {
        return false;
    }
...
export function canChangeMinimumBet(state) {
...
export function setTableMinimumBet(state, amount) {
...
export function logDealBlockedAudit(state, reason) {
    const payload = {
...
export function getDealBlockReason(state) {
...
export function logDealCardsAudit(state, extra) {
