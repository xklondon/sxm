...
import { getChipPlacementTarget, getChipPlacementTargetFromBoxId, placeBetPayloadFromTarget, resolveChipTrayBetTarget, } from './chipPlacement';
...
describe('getChipPlacementTargetFromBoxId (online)', () => {
    ...
    it('throws Box not found for an unknown boxId online', () => {
        const state = createNewBlackjackTable();
        expect(() => getChipPlacementTargetFromBoxId(state, 'client-only-box-id', true)).toThrow(/Box not found/);
    });
...
describe('resolveChipTrayBetTarget', () => {
    ...
    it('uses last slot target when selection is empty', () => {
        const state = createNewBlackjackTable();
        const target = resolveChipTrayBetTarget(state, 'Host', { kind: 'slot', slotNumber: 1 }, true);
