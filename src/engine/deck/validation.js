import { createStandardDeck, drawCard, drawCards, getRemainingCardCount, shuffleDeck, } from './deck';
export function validateDeck(deck) {
    ...
    const remainingIds = new Set(deck.drawOrder.map((i) => deck.cards[i]?.id).filter(Boolean));
    for (const dealtId of deck.dealtCardIds) {
...
export function runDeckEngineChecks() {
    const results = [];
    ...
    results.push({
        ...
        passed: shuffled.cards.length === 52 &&
            shuffledIds.size === 52 &&
            shuffledValid.valid,
    });
