...
function ownerEmailFromMeta(state) {
...
export function bankRepresentsTableOwner(state, bankId) {
...
function resolveBankWinLoserId(state, bankId) {
    const candidates = listPersonBankrollOwnerIds(state).filter((id) => id !== bankId && isHumanPlayer(state, id));
    const broke = candidates.filter((id) => getLedgerBalanceForBankrollOwner(state, id) <= 0);
    if (broke.length === 1) {
        return broke[0];
    }
...
const IOU_MISSING_EMAIL_MESSAGE = 'Cannot create IOU because one player is missing an email address.';
/** Hint when Create IOU is disabled on the game-over overlay. */
export function getGameEndIouDisabledReason(state) {
    if (state.tableMeta.gameStatus !== 'ended') {
    ...
    if (winnerId && isHumanPlayer(state, winnerId)) {
        const loserId = winnerId === bankId && bankId
